...
def build_subject(code, name, cap, rows, version, kind):
    ...
    for r in rows:
        ...
        if parent and parent.lower() != text.lower() and parent != groups[gk]['title']:
            text = f'{parent} — {text}'
        # Aeroplane column only. This is a PPL(A) app; the helicopter objectives
        # (rotor heads, transmission, blades, turbine engines, heliport performance,
        # rotor emergencies) are not examined and counting them inflates every
    ...
    return {'code': code, 'name': name, 'cap': cap, 'kind': kind,
            ...
            'groups': [groups[k] for k in order if groups[k]['items']]}
...
def main():
    ...
    for code, name, cap, dl_id, kind in SUBJECTS:
        ...
        print(f'{code} {name:32s} groups={len(subject["groups"]):3d} '
              f'LOs={n:4d} {subject["version"]}', file=sys.stderr)
        out.append(subject)

    total = sum(len(g['items']) for s in out for g in s['groups'])
    print(f'\ntotal {total} PPL(A) learning objectives', file=sys.stderr)

    with open(OUT, 'w') as f:
        f.write('/* UK CAA PPL(A) theoretical knowledge learning objectives, generated by\n'
                '   tools/build_syllabus.py from the CAA CAP2090 documents.\n'
                '   Aeroplane objectives only — the helicopter column is dropped. */\n')
